...
//|   - STYLE: STYLE_DOT                                             |
//|   - WIDTH: 1                                                     |
//|   - Default ColorAbove=White, ColorBelow=Brown.                  |
//+------------------------------------------------------------------+
...
input color            ColorAbove      = clrWhite;       // Color when dropped ABOVE Bid
input color            ColorBelow      = clrBrown;       // Color when dropped BELOW Bid
...
void OnStart()
  {
   ...
   if(!ObjectCreate(ChartID(), name, OBJ_TREND, 0,
     ...
     {
      Print("QL_Dot: failed to create object '", name, "'. Error=", GetLastError());
      return;
     }
   ...
   Print("QL_Dot placed @ ", DoubleToString(price, Digits),
