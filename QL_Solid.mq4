...
//|   - STYLE: STYLE_SOLID                                           |
//|   - WIDTH: 1                                                     |
//|   - Default ColorAbove=Lime, ColorBelow=Magenta.                 |
//+------------------------------------------------------------------+
...
input color            ColorAbove      = clrLime;       // Color when dropped ABOVE Bid
input color            ColorBelow      = clrMagenta;    // Color when dropped BELOW Bid
...
void OnStart()
  {
   ...
   if(!ObjectCreate(ChartID(), name, OBJ_TREND, 0,
     ...
     {
      Print("QL_Solid: failed to create object '", name, "'. Error=", GetLastError());
      return;
     }
   ...
   Print("QL_Solid placed @ ", DoubleToString(price, Digits),
