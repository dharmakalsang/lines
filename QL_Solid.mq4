...
#property description "QL_Solid - Solid line, Width 1"
...
input color            ColorAbove      = clrLime;
input color            ColorBelow      = clrMagenta;
...
bool GetDropLocation(double &price, datetime &dropTime, int &dropBar)
  {
   ...
   if(dropTime <= 0)
     {
      ...
      Print("QL_Solid - Solid line, Width 1: dropped outside chart area; using current bar time fallback.");
     }
   ...
   if(price <= 0.0)
     {
      Print("QL_Solid - Solid line, Width 1: invalid price at drop location, aborting.");
      return(false);
     }
   ...
   if(dropBar < 0 || dropBar >= maxBars)
     {
      Print("QL_Solid - Solid line, Width 1: invalid drop bar index ", dropBar, ", aborting.");
      return(false);
     }
...
int OnInit()
  {
   ...
   Print("QL_Solid - Solid line, Width 1 attached. Current throw count = ", cur,
   ...
   if(ResetOnRemove && reason == REASON_REMOVE)
     {
      int cur = CurrentCounter();
      ResetCounter();
      Print("QL_Solid - Solid line, Width 1 removed. Counter reset (was ", cur, "). Next attach will start at 1.");
     }
   else
     {
      int cur = CurrentCounter();
      Print("QL_Solid - Solid line, Width 1 deinit (reason=", reason, "). Counter preserved = ", cur);
     }
  }
...
void OnStart()
  {
   ...
   if(leftBar <= rightBar)
     {
      Print("QL_Solid - Solid line, Width 1: not enough history to create a line of the requested length.");
      return;
     }
   ...
   if(!ObjectCreate(ChartID(), name, OBJ_TREND, 0,
     ...
     {
      Print("QL_Solid - Solid line, Width 1: failed to create object '", name, "'. Error=", GetLastError());
      return;
     }
   ...
   Print("QL_Solid - Solid line, Width 1 placed @ ", DoubleToString(price, Digits),
