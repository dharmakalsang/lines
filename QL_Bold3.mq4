...
input color            ColorAbove      = clrBlue;
input color            ColorBelow      = clrRed;
...
bool GetDropLocation(double &price, datetime &dropTime, int &dropBar)
  {
   ...
   if(dropTime <= 0)
     {
      ...
      Print("QL_Bold3 - Main Support/Resistance line (Solid, Width 3): dropped outside chart area; using current bar time fallback.");
     }
   ...
   if(price <= 0.0)
     {
      Print("QL_Bold3 - Main Support/Resistance line (Solid, Width 3): invalid price at drop location, aborting.");
      return(false);
     }
   ...
   if(dropBar < 0 || dropBar >= maxBars)
     {
      Print("QL_Bold3 - Main Support/Resistance line (Solid, Width 3): invalid drop bar index ", dropBar, ", aborting.");
      return(false);
     }
...
int OnInit()
  {
   ...
   Print("QL_Bold3 - Main Support/Resistance line (Solid, Width 3) attached. Current throw count = ", cur,
   ...
   if(ResetOnRemove && reason == REASON_REMOVE)
     {
      int cur = CurrentCounter();
      ResetCounter();
      Print("QL_Bold3 - Main Support/Resistance line (Solid, Width 3) removed. Counter reset (was ", cur, "). Next attach will start at 1.");
     }
   else
     {
      int cur = CurrentCounter();
      Print("QL_Bold3 - Main Support/Resistance line (Solid, Width 3) deinit (reason=", reason, "). Counter preserved = ", cur);
     }
  }
...
void OnStart()
  {
   ...
   if(leftBar <= rightBar)
     {
      Print("QL_Bold3 - Main Support/Resistance line (Solid, Width 3): not enough history to create a line of the requested length.");
      return;
     }
   ...
   if(!ObjectCreate(ChartID(), name, OBJ_TREND, 0,
     ...
     {
      Print("QL_Bold3 - Main Support/Resistance line (Solid, Width 3): failed to create object '", name, "'. Error=", GetLastError());
      return;
     }
   ...
   Print("QL_Bold3 - Main Support/Resistance line (Solid, Width 3) placed @ ", DoubleToString(price, Digits),
