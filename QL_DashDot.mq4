...
//|   - STYLE: STYLE_DASHDOT                                         |
//|   - WIDTH: 1                                                     |
//|   - Default ColorAbove=Aqua, ColorBelow=Silver.                  |
//+------------------------------------------------------------------+
...
input color            ColorAbove      = clrAqua;        // Color when dropped ABOVE Bid
input color            ColorBelow      = clrSilver;      // Color when dropped BELOW Bid
...
void OnStart()
  {
   ...
   if(!ObjectCreate(ChartID(), name, OBJ_TREND, 0,
     ...
     {
      Print("QL_DashDot: failed to create object '", name, "'. Error=", GetLastError());
      return;
     }
   ...
   Print("QL_DashDot placed @ ", DoubleToString(price, Digits),
